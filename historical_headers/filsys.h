...
#include <sys/param.h>  /* definitions of basic system constants           */
#include <sys/types.h>  /* definitions of basic kernel data types          */
...
{       /* basic file system parameters - initialized when FS is created   */
	...
	ushort  s_isize;        /* size (in blocks) of i-list & overhead   */
	...
	short   s_sicino;       /* byte offset to start of free i-node lst */
	...
	short   s_sicfrag;      /* byte offset to start of frag table      */
	daddr_t	s_swaplo;	/* start of swap area */
	daddr_t	s_nswap;	/* number of block of swap area */
...
#define s_rsvd		s_bcun.unused
	...
	ushort  s_tffrag;       /* number of fragmented files              */
	ushort  s_tbfrag;       /* number of fragmented blocks             */
	short   s_findex;       /* fragment allocation index               */
	char    s_fmod;         /* flag: set when file system is mounted   */
                                /* see below                               */
	char    s_ronly;        /* flag: file system is read only          */
	daddr_t s_tfree;        /* total number of available free blocks   */
	char    s_flock;        /* flag: free list lock                    */
	char    s_ilock;        /* flag: free i-node list lock             */
	short   s_nfree;        /* number of valid blocks in free-list     */
	ino_t   s_tinode;       /* total number of available i-nodes       */
	short   s_ninode;       /* number of valid i-nodes in free-list    */
